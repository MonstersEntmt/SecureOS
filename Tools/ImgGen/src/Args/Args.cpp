...
namespace Args
{
	...
	static struct AutoRegister
	{
		AutoRegister()
		{
			...
			RegisterOption({ .Name = "version" }, { .Description = "Show version info in format 'ImgGen major.minor.patch'" });
		}
	} s_AutoRegister;
	...
	std::vector<std::string>& Option::operator[](size_t index)
	{
		...
		{
			std::cerr << "ImgGen ERROR: attempting to access non existent option '" << index << "'\n";
			exit(1);
		}
	...
	Option& Options::operator[](std::string_view name)
	{
		...
		std::cerr << "ImgGen ERROR: attempting to access non existent option '" << name << "'\n";
		exit(1);
	}
	...
	Options Handle(int argc, const char* const* argv)
	{
		...
		for (size_t i = 1; i < argc; ++i)
		{
			...
			if (arg[0] == '-')
			{
				if (arg.size() == 1)
				{
					std::cerr << "ImgGen ERROR: invalid empty option '-' specified\n";
			...
			{
				if (isDefaultOption)
					std::cerr << "ImgGen ERROR: given argument for default option, but that is not allowed\n";
				else
					std::cerr << "ImgGen ERROR: unknown option '" << arg << "' specified\n";
				options.ShouldExit = true;
				options.ExitCode   = 1;
				break;
			}
			...
			{
				std::cerr << "ImgGen ERROR: missing '" << (option.MinArgs - (argc - i - 1)) << "' arguments for option '--" << option.Name << "'\n";
				options.ShouldExit = true;
				options.ExitCode   = 1;
				break;
			}
			...
			if (!isDefaultOption)
			{
				...
				{
					std::cerr << "ImgGen ERROR: missing '" << (option.MinArgs - argCount) << "' arguments for option '--" << option.Name << "'\n";
					options.ShouldExit = true;
					options.ExitCode   = 1;
					break;
				}
				...
				{
					std::cerr << "ImgGen ERROR: '" << (argCount - option.MaxArgs) << "' too many arguments for option '--" << option.Name << "'\n";
					options.ShouldExit = true;
					options.ExitCode   = 1;
					break;
				}
			...
			for (auto& option : options["help"].Options)
			{
				...
				else
				{
					...
					for (auto& opt : option)
					{
						...
						{
							std::cout << "ImgGen WARN: help for option " << opt << " not found\n";
						}
					}
				}
			}
		...
		else if (!options.ShouldExit && defaultArgI != s_Options.size())
		{
			...
			{
				std::cerr << "ImgGen ERROR: missing '" << (s_Options[defaultArgI].first.MinArgs - argCount) << "' arguments for default option\n";
				options.ShouldExit = true;
				options.ExitCode   = 1;
			}
			if (argCount > s_Options[defaultArgI].first.MaxArgs)
			{
				std::cerr << "ImgGen ERROR: '" << (argCount - s_Options[defaultArgI].first.MaxArgs) << "' too many arguments for default option\n";
				options.ShouldExit = true;
				options.ExitCode   = 1;
			}
		}
